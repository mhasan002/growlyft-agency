...
export default function Blog() {
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      {/* Search Results or Latest Articles Section */}
      <section className="py-12 bg-white">
        <div className="container mx-auto px-6">
          <div className="flex justify-between items-center mb-8">
            <h2 className="text-2xl font-bold text-gray-900">
              {searchQuery || selectedCategory !== "All" ? "Search Results" : "Latest"}
            </h2>
            <Button 
              variant="outline"
            ...
            >
              {searchQuery || selectedCategory !== "All" ? "Clear Filters" : "View all"}
            </Button>
          </div>

          {filteredPosts.length > 0 ? (
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 mb-16">
              {filteredPosts.slice(0, searchQuery || selectedCategory !== "All" ? filteredPosts.length : 6).map((post: BlogPost) => (
                <Link key={post.id} href={`/blog/${post.slug}`} className="group">
                  <div className="bg-white hover:shadow-lg transition-shadow duration-300 rounded-lg overflow-hidden shadow-sm">
                    <img 
